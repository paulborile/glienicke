...
func TestFilterMatching(t *testing.T) {
	...
	evt1, kp1 := testutil.MustNewTestEvent(1, "Event 1", nil)

	evt2, _ := testutil.NewTestEventWithKey(kp1, 2, "Event 2", nil)

	evt3, kp2 := testutil.MustNewTestEvent(1, "Event 3", [][]string{{"e", evt1.ID}})

	evt4, _ := testutil.NewTestEventWithKey(kp2, 3, "Event 4", [][]string{{"p", kp1.PubKeyHex}})



	events := []*event.Event{evt1, evt2, evt3, evt4}

	for _, evt := range events {

		if err := client.SendEvent(evt); err != nil {

			t.Fatalf("Failed to send event %s: %v", evt.ID, err)

		}

		if _, _, err := client.ExpectOK(evt.ID, 2*time.Second); err != nil {

			t.Fatalf("Failed to receive OK for event %s: %v", evt.ID, err)
	...
	testCases := []struct {
...

		{
			...
			filter:         &event.Filter{IDs: []string{evt1.ID, evt3.ID}},

			expectedEvents: []*event.Event{evt1, evt3},
...

		{
			...
			expectedEvents: []*event.Event{evt1, evt2},
...

		{
			...
			filter:         &event.Filter{Kinds: []int{1, 3}},

			expectedEvents: []*event.Event{evt1, evt3, evt4},
...

								{
									...
									expectedEvents: []*event.Event{evt3},
...

								{
									...
									expectedEvents: []*event.Event{evt4},
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			subID := fmt.Sprintf("sub-%s", tc.name)

			if err := client.SendReq(subID, tc.filter); err != nil {
			...
			for i := 0; i < len(tc.expectedEvents); i++ {
				...
				evt, err := client.ExpectEvent(subID, 2*time.Second)
			...
			for _, expected := range tc.expectedEvents {
				...
				if !receivedEvents[expected.ID] {

					t.Errorf("Expected event %s was not received", expected.ID)
			...
			if len(receivedEvents) != len(tc.expectedEvents) {

				t.Errorf("Expected %d events, but got %d", len(tc.expectedEvents), len(receivedEvents))
